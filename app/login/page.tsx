    ...
    <div style={{ minHeight: "100vh", background: "#F7F4EF", display: "flex", flexDirection: "column" }}>
      <header style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <Link href="/registro" style={{ fontSize: 14, color: "#6B6259", textDecoration: "none" }}>
          ¿No tienes cuenta? <span style={{ color: "#1B3F2F", fontWeight: 600 }}>Regístrate gratis</span>
        </Link>
      </header>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 24px 48px" }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          <button
            ...
            title={!SUPABASE_CONFIGURED ? "Google login requiere configurar Supabase" : undefined}
            style={{
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
              width: "100%", padding: "12px 16px", borderRadius: 12,
              border: "1.5px solid rgba(26,22,18,0.14)", background: "#FFFFFF",
              fontSize: 15, fontWeight: 600, color: "#1A1612",
              cursor: !SUPABASE_CONFIGURED ? "not-allowed" : "pointer",
              marginBottom: 20, opacity: (googleLoading || !SUPABASE_CONFIGURED) ? 0.45 : 1,
            }}
          >
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#1A1612" }}>Contraseña</label>
                <button
                  type="button"
                  onClick={handleForgotPassword}
                  style={{ fontSize: 12, color: "#1B3F2F", background: "none", border: "none", cursor: "pointer", padding: 0, fontWeight: 600 }}
                >
                  ¿Olvidaste la tuya?
                </button>
              </div>
