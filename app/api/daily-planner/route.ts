  ...
  for (const s of subjects) {
    ...
    if (goalType === "hours" && goalValue) {
      ...
      if (!isNaN(hoursTarget) && hoursTarget > 0) {
        tasks.push({
          ...
          description: `Meta: ${hoursTarget}h/semana. Dedicá tiempo hoy para cumplir tu objetivo.`,
    ...
    if (daysToExam !== null && daysToExam <= 3) {
      tasks.push({
        ...
        description: daysToExam === 0 ? "¡El examen es hoy!" : daysToExam === 1 ? "El examen es mañana." : `Faltan ${daysToExam} días. Revisá los conceptos críticos.`,
        minutesEst: 25,
        priority: 1,
    ...
    if (total > 0 && coveragePct < 60 && due === 0) {
      tasks.push({
        ...
        description: `Solo dominás el ${coveragePct}% del temario. Un quiz identifica tus puntos débiles.`,
        minutesEst: 8,
